...
public class FaceNet implements Closeable {
	public static class FaceNetDetectedFace {
	...
	public List<FaceNetDetectedFace> detectFaces(MBFImage img) {
		...
		try {
			...
			final NDArray<double[]> arr = (NDArray<double[]>) result.get(1);
			probs = arr.getData();

			for (int i = 0; i < boxes.length; i++) {
				final FaceNetDetectedFace det = new FaceNetDetectedFace(boxes[i], (float) probs[i]);
				detections.add(det);
	...
	public static void main(String[] args) throws JepException, IOException {
		...
		for (final MBFImage img : vc) {
			...
			for (final FaceNetDetectedFace r : dets) {
				...
				if (d1 < 0.5 && d2 < 0.5) {
					img.drawText("Jon", r.bounds.getTopLeft(), HersheyFont.ROMAN_SIMPLEX,
							14);
				}
			}
